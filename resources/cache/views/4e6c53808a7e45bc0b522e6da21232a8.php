...
<div class="min-h-screen flex flex-col md:flex-row bg-white dark:bg-gray-900">
    ...
    <div class="md:w-1/2 flex items-center justify-center p-8 md:p-12 lg:p-24">
        <div class="w-full max-w-md mt-12">
            ...
            <!-- Flash Message -->
            <div class="mt-4"><?php echo flash()->display('#form-reset-password'); ?></div>

            <!-- Reset Password Form -->
            <form id="form-reset-password" action="<?php echo htmlspecialchars((string)(route('submit.reset.password')), ENT_QUOTES, 'UTF-8'); ?>" method="POST" class="space-y-6">
                <?= csrf_field() ?>

                <input type="hidden" name="email" value="<?php echo htmlspecialchars((string)($email ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
