    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-2/3">
                ...
                <div class="mt-4">
                    @flash_info('#form-submit-thread')
                </div>

                <!-- Thread Form -->
                <div class="bg-white">
                    <form id="form-submit-thread" action="{{route('submit.thread')}}" method="POST" enctype="multipart/form-data">
                        {!! csrf_field() !!}
                        <!-- Category Selection -->
                        <div class="mb-6">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                ...
                                @foreach ($categories as $key => $cat)
                                    <div>
                                        <input type="radio" name="category" value="{{ $key }}" id="category-{{ $key }}"
                                            class="peer hidden" {{ $selected === $key ? 'checked' : '' }}>
                                        ...
                                        <label for="category-{{ $key }}"
                                            ...
                                            <div class="flex items-start h-12">
                                                <div class="{{ $cat['bg'] }} p-1 rounded-lg mr-2">
                                                    {!! $cat['icon'] !!}
                                                </div>
                                                <div>
                                                    <h3 class="text-xs font-medium text-gray-800 mb-1">{{ $cat['label'] }}</h3>
                                                    <p class="text-xs text-gray-400">{{ $threadCounts[$key] ?? 0 }} Threads</p>
                                                </div>
                                            </div>
                                        </label>
                                    </div>
